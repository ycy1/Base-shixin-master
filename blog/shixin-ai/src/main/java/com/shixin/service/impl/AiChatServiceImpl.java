...
import com.shixin.entity.ConfigSource;
import com.shixin.mapper.AgentMapper;
import com.shixin.mapper.ConfigSourceMapper;
import com.shixin.mapper.ConversationMapper;
import com.shixin.mapper.MemoryMapper;
import com.shixin.service.AiChatService;
...
public class AiChatServiceImpl implements AiChatService {
    ...
    private final com.shixin.kit.DatabaseKnowledgeLoader databaseKnowledgeLoader;
    private final com.shixin.service.ChatCommandService chatCommandService;
    ...
    public Flux<String> sendMessage(ChatDto dto) {
        ...
        return Flux.create(sink -> {
            try {
                ...
                if (com.shixin.enums.ChatCommandEnum.isCommand(userContent)) {
                    var parseResult = com.shixin.enums.ChatCommandEnum.parse(userContent);
                    if (parseResult != null) {
     ...
     * 处理聊天指令
     */
    private String handleCommand(com.shixin.enums.ChatCommandEnum.ParseResult parseResult,
    ...
    private String generateAndExecuteSql(Agent agent, java.util.List<dev.langchain4j.data.message.ChatMessage> messages, String userContent) {
        ...
        try {
            com.shixin.entity.ConfigSource dbConfig = configSourceMapper.selectById(agent.getConfigId());
            ...
            com.shixin.kit.DatabaseKnowledgeLoader.validateReadOnly(sql);
    ...
    private dev.langchain4j.agent.tool.ToolSpecification[] parseAgentTools(String toolsJson) {
        ...
        try {
            ...
            for (String className : classNames) {
                try {
                    Class<?> clazz = Class.forName("com.shixin.tools." + className);
                    specs.addAll(dev.langchain4j.agent.tool.ToolSpecifications.toolSpecificationsFrom(clazz));
                } catch (ClassNotFoundException e) {
                    log.warn("工具类不存在: com.shixin.tools.{}", className);
                }
            }
        ...
        try {
            ...
            for (Class<?> clazz : toolClasses) {
                for (var method : clazz.getDeclaredMethods()) {
                    ...
                    String resultStr = (result instanceof com.shixin.utils.ToolsResult)
                            ? ((com.shixin.utils.ToolsResult) result).getData().toString()
        ...
        try {
            ...
            while (resources.hasMoreElements()) {
                ...
                for (java.io.File file : dir.listFiles()) {
                    ...
                    String className = "com.shixin.tools." + file.getName().replace(".class", "");
